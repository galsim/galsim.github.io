...
class gg_gplus_integration {
	...
	private $scope = "https://picasaweb.google.com/data/";
	...
	public function __construct($connect_id, $username = false) {
		...
		if(empty($connect_id)) {
			...
			$this->g_username = gg_get_arr_key($conn_data, 'gplus_user');
		}
	...
	private function curl_call($url, $params = false, $with_header = true) {
	    ...
	    if($with_header) {
			curl_setopt($ch, CURLOPT_HTTPHEADER, 
				array('Content-Type: application/json','Authorization: Bearer '. $this->real_token, 'X-JavaScript-User-Agent: Global Gallery', 'GData-Version: 2')
			);
		}
		if($params) {
	...
	public function base_tokens_db($action, $tokens = array('base' => '', 'refresh' => '')) {
		$db = (array)get_option('gg_gplus_base_tokens_db', array());
	...
	public function get_albums() {
		$url = 'https://picasaweb.google.com/data/feed/api/user/'. urlencode($this->g_username);
